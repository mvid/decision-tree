...
public class Dataset implements Writable {
   ...
   * Loads the dataset from a file
   */
  public static Dataset loadWithScanner(Scanner path) throws IOException {

    Dataset dataset = readWithScanner(path);
  ...
  public static Dataset readWithScanner(Scanner in) throws IOException {
    Dataset dataset = new Dataset();
    
    dataset.readFieldsWithScanner(in);
    return dataset;
  }
  ...
  public void readFieldsWithScanner(Scanner in) throws IOException {
    int nbAttributes = in.nextInt();
    in.nextLine();
    attributes = new Attribute[nbAttributes];
    for (int attr = 0; attr < nbAttributes; attr++) {
    ...
    String[] ignoredStr = in.nextLine().split(",");
    if(ignoredStr[0].equals("none"))
        ignored = new int[0];
    else{
        ignored = new int[ignoredStr.length];
        for(int i = 0; i < ignored.length; i++){
            ignored[i] = Integer.parseInt(ignoredStr[i]);
        }
    }

    System.err.println(ignored.length);

    // only CATEGORICAL attributes have values
    values = new String[nbAttributes][];
    for (int attr = 0; attr < nbAttributes; attr++) {
      if (attributes[attr].isCategorical()) {
        values[attr] = in.nextLine().split(",");
      }
    }
    labelId = in.nextInt();
    in.nextLine();
    nbInstances = in.nextInt();
  }
